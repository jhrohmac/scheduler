...
//	protected String seedKeyStr = "";
//	protected String encData = "";
//	protected String plainData = "";
//	protected String decData = "";
//	public String getseedKeyStr() {
//		return seedKeyStr;
//	}
//	public void setseedKeyStr(String val) {
//		seedKeyStr = val;
//	}
...
//		return encData;
//	}
//	public void setencData(String val) {
//		encData = val;
//	}
//	public String getdecData() {
//		return decData;
//	}
//	public void setdecData(String val) {
//		decData = val;
//	}
/****** END SET/GET METHOD, DO NOT MODIFY *****/
	public static final String[][] HELP_STRINGS = {
		/*
		 * 도움말 작성 { <function name>, <help string>, <example input 1>,
		 * <exampleoutput 1>, <example input 2>, <example output 2>, ...}
		 */
		{ "invokeEncrypt", "평문을 암호화합니다.",
			"invokeEncrypt('암호화 키', '암호화할 문자열', '인코딩')", "d3inlFkAPJ/vnTi2Ejyb7dbwU84O/GdBYTnWfo62F0U=" },
		{ "invokeDecrypt", "암호화된 문자열을 평문으로 복호화합니다.",
			"invokeDecrypt('암호화 키', '복호화할 문자열', '인코딩')", "ABCD1234abcd!@#$한글닷컴" },
	};
	...
	static long[] RoundKey;
	
	public SeedUtil() {
	}

	// 암호화 Util
	public static String getSeedEncData(String strPlain) {
		String strRtn = "";
		try {
			byte[] bEncBuffer = null;
			byte[] bDecBuffer = strPlain.getBytes();

			bEncBuffer = encrypt(bDecBuffer);

			strRtn = Base64.getEncoder().encodeToString(bEncBuffer);
		} catch (Exception localException) {
			localException.printStackTrace();
		}
		return strRtn;
	}
	public static String getSeedEncData(String strPlain, String encType) {
		String strRtn = "";
		try {
			byte[] bEncBuffer = null;
			byte[] bEncBuff = strPlain.getBytes(encType);

			bEncBuffer = encrypt(bEncBuff);
	...
	// 복호화 Util
	public static String getSeedDecData(String encStr) {
		String strRtn = "";
		try {
			byte[] bEncBuff = Base64.getDecoder().decode(encStr);
			byte[] bDecBuff = decrypt(bEncBuff);

			strRtn = new String(bDecBuff);
		} catch (Exception localException) {
			localException.printStackTrace();
		}
		return strRtn;
	}
	public static String getSeedDecData(String encStr, String encType) {
		String strRtn = "";
		try {
			encStr = new String(encStr.getBytes(encType));

			byte[] bEncBuff = Base64.getDecoder().decode(encStr);

			byte[] bDecBuff = decrypt(bEncBuff);

			strRtn = new String(bDecBuff, encType);
		} catch (Exception localException) {
			localException.printStackTrace();
		}
		return strRtn;
...
//	}
	...
	public static void Seedx(byte[] key) {
		RoundKey = new long[32];
		seedEncRoundKey(key);
	}

	private static byte getAByteLong(long src, int index) {
		return (byte) (int) (src >>> index * 8 & 0xFF);
	}

	private static long getAByteToLong(long src, int index) {
		return src >> index * 8 & 0xFF;
	}

	private static long makeLong(byte[] byteArray, int nStartIndex) {
		long b1 = byteArray.length > nStartIndex ? (0xFF & byteArray[(nStartIndex + 0)]) << 24
				: 0L;

		long b2 = byteArray.length > nStartIndex + 1 ? (0xFF & byteArray[(nStartIndex + 1)]) << 16
				: 0L;

		long b3 = byteArray.length > nStartIndex + 2 ? (0xFF & byteArray[(nStartIndex + 2)]) << 8
				: 0L;

		long b4 = byteArray.length > nStartIndex + 3 ? 0xFF & byteArray[(nStartIndex + 3)]
				: 0L;

		return b1 + b2 + b3 + b4;
	}

	private static long[] encRoundKeyUpdate0(long[] keyData, int roundCnt) {
		long T0 = keyData[0];
		keyData[0] = (keyData[0] >> 8 & 0xFFFFFFFF ^ keyData[1] << 24 & 0xFFFFFFFF);
		keyData[1] = (keyData[1] >> 8 & 0xFFFFFFFF ^ T0 << 24 & 0xFFFFFFFF);
		T0 = keyData[0] + keyData[2] - KC[roundCnt] & 0xFFFFFFFF;
		long T1 = keyData[1] + KC[roundCnt] - keyData[3] & 0xFFFFFFFF;

		RoundKey[(roundCnt * 2)] = (SS0[((int) getAByteToLong(T0, 0))]
				^ SS1[((int) getAByteToLong(T0, 1))]
				^ SS2[((int) getAByteToLong(T0, 2))] ^ SS3[((int) getAByteToLong(
				T0, 3))]);

		RoundKey[(roundCnt * 2 + 1)] = (SS0[((int) getAByteToLong(T1, 0))]
				^ SS1[((int) getAByteToLong(T1, 1))]
				^ SS2[((int) getAByteToLong(T1, 2))] ^ SS3[((int) getAByteToLong(
				T1, 3))]);

		return keyData;
	}

	private static long[] encRoundKeyUpdate1(long[] keyData, int roundCnt) {
		long T0 = keyData[2];
		keyData[2] = ((keyData[2] << 8 ^ keyData[3] >> 24) & 0xFFFFFFFF);
		keyData[3] = ((keyData[3] << 8 ^ T0 >> 24) & 0xFFFFFFFF);
		T0 = keyData[0] + keyData[2] - KC[roundCnt] & 0xFFFFFFFF;
		long T1 = keyData[1] + KC[roundCnt] - keyData[3] & 0xFFFFFFFF;
	...
	private static void seedEncRoundKey(byte[] pbUserKey) {
	...
	private static long[] seedRound(long[] data, int roundIndex,
			boolean swapFlag) {
		long R1;
		long R0;
		// long R1;
		...
		return !swapFlag ? new long[] { L0, L1, R0, R1 } : new long[] { R0, R1,
				L0, L1 };
	}

	public static byte[] encrypt(byte[] plainData) {
		int nLoopCnt = plainData.length / 16
				+ (plainData.length % 16 != 0 ? 1 : 0);
	...
	private static long[] encrypt16(long[] plainData) {
		...
		return new long[] { plainData[2], plainData[3], plainData[0],
				plainData[1] };
	}

	public static byte[] decrypt(byte[] cipherData) {
		int nLoopCnt = cipherData.length / 16;
		byte[] retArray = new byte[cipherData.length];
	...
	private static long[] decrypt16(long[] cipherData) {
		boolean bSwapFlag = false;
	...
	public static String invokeDecrypt(String seedKeyStr, String encData, String encode) {
/* Available Variables: DO NOT MODIFY
	In  : String seedKeyStr
	In  : String encData
	Out : String decData
* Available Variables: DO NOT MODIFY *****/

		// KEY 설정
		byte[] seedKeyByte = Base64.getDecoder().decode(seedKeyStr);

		Seedx(seedKeyByte);

		// 암호화
		//encData = getSeedEncData(plainData);

		return getSeedDecData(encData, encode).trim();
	}

	public static String invokeEncrypt(String seedKeyStr, String plainData, String encode) {
/* Available Variables: DO NOT MODIFY
	In  : String seedKeyStr
	In  : String plainData
	Out : String encData
* Available Variables: DO NOT MODIFY *****/
		...
		return getSeedEncData(plainData, encode);

		// 복호화
		//String decData = getSeedDecData(encData).trim();
	}
	...
	public static void main(String[] args) {
		...
		String dd ="그 후에는 Base64를 사용하여 Seed를 인코딩하고 디코딩하는 메서드를 구현했습니다. 이는 단순한 예제이며, 실제로 보안 관련 작업에서는 보다 안전한 난수 생성 방법을 사용하는 것이 좋습니다.";
		dd ="1234";
		try {
			String result1 = SeedUtil.invokeEncrypt("tlsfk!2016", dd, "UTF-8");
			System.out.println(result1);
			
			String result2 = SeedUtil.invokeDecrypt("tlsfk!2016", result1, "UTF-8"); 
