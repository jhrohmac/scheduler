...
class BullishMomentumDetector {
    constructor(options) {
        this.options = {
            consecutiveBullDays: (options && options.consecutiveBullDays) || 3,
            longCandleRatio: (options && options.longCandleRatio) || 2.0,
            apiEndpoint: (options && options.apiEndpoint) || '/scheduler/finance/detectBullishMomentum.do'
        };
    }
    ...
    detectLocalFromStockData(candles, stockCode, stockName) {
        if (!candles || candles.length < 240) {
            console.warn(`충분한 캔들 데이터 필요: ${(candles && candles.length) || 0} / 240`);
            return [];
        }
    ...
    renderSignalsTable(signals, containerId) {
        ...
        signals.forEach(signal => {
            ...
            const maInfo = `
                5: ${signal.ma5 != null ? signal.ma5.toFixed(0) : ''}<br/>
                20: ${signal.ma20 != null ? signal.ma20.toFixed(0) : ''}<br/>
                60: ${signal.ma60 != null ? signal.ma60.toFixed(0) : ''}<br/>
                120: ${signal.ma120 != null ? signal.ma120.toFixed(0) : ''}<br/>
                240: ${signal.ma240 != null ? signal.ma240.toFixed(0) : ''}
            `;
            ...
            html += `
                <tr>
                    ...
                    <td>${signal.closePrice != null ? signal.closePrice.toFixed(0) : ''}</td>
                    <td>${typeLabel}</td>
                    <td>${details}</td>
