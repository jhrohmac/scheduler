...
<script src="/scheduler/finance/stock/websocket/crypto-js.min.js"></script>
<!-- 화면분할을 위한 div 처리 -->
<style>
	div.row {
			width: 100%;
...
<script>
	...
	/* appkey 입력 변수 */
	var g_app_key="PSJpD5cV7aIZ4Dvp1vOrHqvJ0J1rxyI41lca";

	/* 개인용 appsecret 입력 변수 */
	var g_appsecret="W/G/VS31iG0GUnzo9EQUnToAE1NdS2XMqBooOXeNT67rSNbSXKdIMfF71wfsuNPvIcpVvNSmqhEGcg2csOcl5ZoMNbYubloDkc1xePzT0j2kVSrKh3mioSm3yyXubqy1BHfkhCNECk9Gv+fFFcCgNP3arIHoCKZZpLxd4jctvEc3HUuJgyk=";	//테스트후삭제
	...
	/** start of getApproval function **********************/
	const axios = require('axios');
	async function getApproval(key, secret) {
	    // url = 'https://openapivts.koreainvestment.com:29443'; // 모의투자계좌
	    const url = 'https://openapi.koreainvestment.com:9443'; // 실전투자계좌
	    ...
	    try {
	        ...
	        console.error('Error during approval key retrieval:', error.message);
	        throw error;
	    }
	}
	
	getApproval(g_app_key, g_appsecret)
	    .then(g_approval_key => {
	        console.log('Approval Key:', g_approval_key);
	    })
	    .catch(error => {
	        console.error('Error:', error.message);
	    });
	/** end of getApproval function **********************/
	...
	window.onload = function() {
		...
		document.getElementById("hokaregButton2").onclick = function() {
			...
			if (stockcode_tmp == stockcode2 || stockcode_tmp == stockcode1)
			{
				...
				//var result = '{"header": {"approval_key":""+g_approval_key+"","custtype":"P","tr_type":"1","content-type":"utf-8"},"body": {"input": {"tr_id":"H0STASP0","tr_key":"005930"}}}';
