...
echo "[1/2] Compile position DDL runner"
javac -encoding UTF-8 -cp "$OJDBC_JAR" -d "$OUT" "$WORK/DbPositionDdlRunner.java"

echo "[2/2] Apply position DDL (create table/indexes if missing)"
java -cp "$OUT:$OJDBC_JAR" db.tools.DbPositionDdlRunner "$PROP_FILE"
